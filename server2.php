...
//  $gender=$conn->real_escape_string($_POST['gender']);
    ...
    if (empty($_POST['all_user'])){
       ...
       if (empty($lname)){
        ...
        if (count($error) == 0){
        //$rpass=SHA1($rpass1);
        $rpass=password_hash($rpass1, PASSWORD_DEFAULT);
...
if (empty($_POST['all_user'])){
   ...
   $lcaterr="Please select your category!";
    ...
    if($count==0){
      //$pass=SHA1($pass1);
      $sql1="SELECT Password FROM $lall_user WHERE Email='$email'";
      ...
      $sql="SELECT * FROM $lall_user WHERE Email='$email' AND Password='$hashed_pass'";
      $result=$conn->query($sql);
      if(($result->num_rows) == 1){
        ...
        $perr1="Wrong password";
        $count++;
    }
   }
       else{
         $emerr="Wrong email address";
         $count++;
       }
    }
